...
function goToStep(n) {
  [1, 2].forEach(i => $(`step${i}`).classList.toggle('d-none', i !== n));
...
const postOtp = (url) =>
  fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: getRegBody() });
...
async function sendOtp() {
  ...
  try {
    const res  = await postOtp(API_SEND_OTP);
...
async function verifyOtp() {
  ...
  try {
    const res  = await fetch(API_VERIFY_OTP, {
      ...
      body: JSON.stringify({ email: getField('verifyEmail'), otp })
    });
    const data = await safeJson(res);
    ...
    if (res.ok) { clearInterval(timerInterval); window.location.href = '/dashboard-delivery/'; return; }
